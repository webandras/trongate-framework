...
final class Core
{
    protected string $current_module = DEFAULT_MODULE;

    protected string $current_controller;

    protected string $current_method = DEFAULT_METHOD;
        ...
        foreach ($url_segments as $url_segment_key => $url_segment_value) {
            ...
            if (is_numeric($pos)) {
                ...
                if (empty($target_dir)) {
                    ...
                    exit('Access denied: assets must be in subdirectories');
                }
                ...
                if (!in_array($first_dir, $allowed_asset_dirs, true)) {
                    http_response_code(403);
                    exit('Access denied: directory not allowed');
                }
                ...
                try {
                    ...
                    if (is_file($asset_path)) {
                        ...
                        if (is_link($asset_path)) {
                            http_response_code(403);
                            exit('Access denied: symbolic links not allowed');
                        }
                        ...
                        if (empty($file_extension)) {
                            http_response_code(403);
                            exit('Access denied: no file extension');
                        }
                        ...
                        if (in_array($file_extension, $forbidden_extensions, true)) {
                            http_response_code(403);
                            exit('Access denied: forbidden file type');
                        }
                        ...
                        if (in_array($file_name_lower, $sensitive_files, true)) {
                            http_response_code(403);
                            exit('Access denied: sensitive file');
                        }
